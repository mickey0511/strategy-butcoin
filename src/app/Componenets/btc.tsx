...
const Btc: React.FC = () => {
  ...
  const butt_NAV = currentPrice * currentAmount;
  const btc_Yield_Qtd = ((currentPrice - 0.0014) / 0.0014) * 100;
  const btc_Yield_Ytd = ((currentPrice - 0.000004) / 0.000004) * 100;
  const btc_Yield_2024 = ((currentPrice - 0.000004) / 0.000004) * 100;
  ...
  const stats = [
    ...
    { name: "Buttcoin NAV ($M)", value: `$${butt_NAV.toFixed(2)}` },
